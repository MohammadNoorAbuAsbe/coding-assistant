...
public static class ToolHandler
{
    ...
    private const string ReadFunctionDescription = "Read and return the contents of a file. By default the whole file is returned (truncated to the token budget); use optional start_line/end_line to read only a line range. When only start_line is given (no end_line), the range auto-expands to the full enclosing method — useful for diagnostics that reference a method; the result reports the expanded range. Read results stay in the conversation: requesting a range already returned this session for an unchanged file returns a short notice instead of the content. Directories are listed as entries. Binary files are rejected with an error. A missing file reports up to three similarly named files as suggestions.";
    ...
    private const string EditFunctionDescription = "Edit a file by performing a string replacement. Use this to make targeted changes instead of rewriting the entire file. Provide the old text to find and the new text to replace it with. Matching tolerates only leading/trailing whitespace and unicode differences per line (plus, as a last resort, line-sequence similarity) — every line of old_string must actually exist in the file, so never approximate, guess, or invent code you have not read. Line-number prefixes copied from Read output (e.g. '177: code') are stripped automatically, so you can paste the Read output verbatim. Can span multiple lines for larger replacements. CRITICAL: the file must have been Read earlier in this session, or the edit is refused — so Read the target lines first and copy old_string from the Read result; for method-level changes, read the ENTIRE method (wide range) before editing; if you only read a small line range, Read a wider range covering the whole block before editing. On success, the result reports the exact line span that was replaced and the file's before/after line counts — compare these against the location and size you intended; if the span does not match your intent, the replacement landed in the wrong place, so Read that region and retry. A CAUTION note means the match was approximate; an edit whose matched span is far larger than old_string is refused outright. A warning that the file changed on disk means you should Read it again to confirm the result. IMPORTANT: do not double-escape. Write quotes and backslashes plainly in old_string/new_string; only newlines are escaped (as \\n). Never wrap replacement text in extra quotes or JSON punctuation — write exactly the characters that should appear in the file.";
    ...
    private const string DiffFunctionDescription = "Generate a preview of the changes that would be made to a file, WITHOUT writing anything. Compares the current contents of file_path on disk with new_content and returns a unified diff with @@ hunks. IMPORTANT: the file on disk is NEVER modified, and the diff describes your PROPOSED new_content — if the diff looks wrong or contains errors, that reflects your new_content, not the file; re-Read the file to see its actual current state. Keep new_content focused on the region you intend to change — a whole-file new_content produces a huge diff. Use this to inspect a change before applying it with the ApplyPatch or Edit tools. Returns 'No differences' if the content is identical.";
    ...
    public static ChatTool CreateEditTool() =>
        CreateTool(EditFunctionName, EditFunctionDescription, [FilePathPropertyName, "old_string", "new_string"],
            StringProperties(
                ...
                ("replace_all", "Optional: set to 'true' to replace all exact occurrences of old_string. When 'false' (default), an old_string that matches in multiple places is rejected and you should add more surrounding context to make it unique.")));
    ...
    public static ChatTool CreateApplyPatchTool() =>
        CreateTool(ApplyPatchFunctionName, ApplyPatchFunctionDescription, [FilePathPropertyName, "patch"],
            StringProperties(
                ...
                ("patch", "The unified diff to apply. One or more hunks in the format '@@ -start,count +start,count @@' (a bare '@@' separator is also accepted), followed by lines prefixed with ' ' (context), '-' (removed), and '+' (added). No timestamps after the --- / +++ headers. Include 2-5 context lines per hunk for unique matching.")));
    ...
    public class EditFileCall
    {
        ...
        public string? replace_all { get; set; }
    }
