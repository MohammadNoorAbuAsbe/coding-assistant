...
public static class ToolHandler
{
    ...
    private const string EditFunctionDescription = "Edit a file by performing a string replacement. Use this to make targeted changes instead of rewriting the entire file. Provide the old text to find and the new text to replace it with. Matching tolerates only leading/trailing whitespace and unicode differences per line (plus, as a last resort, line-sequence similarity) — every line of old_string must actually exist in the file, so never approximate, guess, or invent code you have not read. Line-number prefixes copied from Read output (e.g. '177: code') are stripped automatically, so you can paste the Read output verbatim. Can span multiple lines for larger replacements. On success, the applied diff is returned. CRITICAL: Read the target lines first and copy old_string from the Read result; for method-level changes, read the ENTIRE method (wide range) before editing; if you only read a small line range, Read a wider range covering the whole block before editing. IMPORTANT: do not double-escape. Write quotes and backslashes plainly in old_string/new_string; only newlines are escaped (as \\n). Never wrap replacement text in extra quotes or JSON punctuation — write exactly the characters that should appear in the file.";
