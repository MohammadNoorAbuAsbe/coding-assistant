...
public static class ToolHandler
{
    ...
    private const string ReadFunctionDescription = "Read and return the ENTIRE contents of a file with line numbers. The whole file is ALWAYS returned — start_line/end_line are accepted for compatibility but ignored. The only exception is an enormous file, which is truncated to the tool-result token budget (the result says so). Read results stay in the conversation: reading an unchanged file a second time returns a short notice instead of the content. Directories are listed as entries. Binary files are rejected with an error. A missing file reports up to three similarly named files as suggestions.";
    ...
    public static ChatTool CreateReadTool() =>
        CreateTool(ReadFunctionName, ReadFunctionDescription, [FilePathPropertyName],
            StringProperties(
                ...
                ("start_line", "Ignored — the whole file is always returned."),
                ("end_line", "Ignored — the whole file is always returned.")));
