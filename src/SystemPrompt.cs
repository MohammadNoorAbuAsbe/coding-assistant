...
3. **Edit** — Edit a file by string replacement. The old text is matched with fuzzy tolerance for whitespace and unicode differences, and as a last resort by line-sequence similarity, so it need not be byte-for-byte exact. Can span multiple lines. On success, the applied diff is returned. Parameters: {""file_path"": ""<path>"", ""old_string"": ""<text>"", ""new_string"": ""<replacement text>""}
4. **ApplyPatch** — Apply a unified diff (patch) to a file, making many changes in one call. Hunks: @@ -start,count +start,count @@ followed by lines prefixed with "" "" (context), ""-"" (removed), and ""+"" (added). No timestamps in headers. Hunks match fuzzily (whitespace differences tolerated). The result reports how each hunk matched and returns the applied diff. Code fences around the patch are ignored. To create a new file, the file must not exist and the patch must contain only ""+"" lines. Parameters: {""file_path"": ""<path>"", ""patch"": ""<unified diff>""}.
